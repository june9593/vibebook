...
description: Three-stage progressive recall of past notes from the user's vibebook session-repo. Use when the user starts work in a project repo — debugging a bug, designing a new feature, or running into an unfamiliar subsystem — to surface relevant chronicles + topics before exploring code. Stage 1 reads only the project's topic list (~5 KB). Stage 2 drills into one topic for chronicle frontmatter. Stage 3 reads the full bodies of the chronicles that match. Trigger on: "let's look at X", "I'm working on Y", "fix this bug", "add this feature", "research how Z works", or any task in a repo where the user has run `vibebook sync` before. When the optional memex CLI is on PATH, atomic-card entries fold in automatically.
---
